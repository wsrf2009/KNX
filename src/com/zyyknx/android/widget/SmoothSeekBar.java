...
public class SmoothSeekBar extends ControlView {
	...
	public void onDraw(Canvas canvas) {
		...
		RectF progress_box = new RectF(m_horizontal_padding, m_vertical_padding
				+ m_gap, m_horizontal_padding + m_current_relative_position,
				m_vertical_padding + m_gap + m_box_rect_height);
		canvas.drawRect(progress_box, m_progress_paint);

		RectF cursor_box = new RectF(m_horizontal_padding
				+ m_current_relative_position, m_vertical_padding,
				m_horizontal_padding + m_current_relative_position
						+ m_cursor_rect_width, m_vertical_padding
						+ m_cursor_rect_height);
		canvas.drawRoundRect(cursor_box, 5, 5, m_cursor_paint);

		RectF rest_box = new RectF(m_horizontal_padding
				+ m_current_relative_position + m_cursor_rect_width,
				m_vertical_padding + m_gap, m_horizontal_padding
						+ m_cursor_rect_width + m_box_rect_width,
				m_vertical_padding + m_gap + m_box_rect_height);
		canvas.drawRect(rest_box, m_rest_paint);

		for (int i = 0; i <= m_total_labels; ++i) {
			canvas.drawCircle(m_horizontal_padding + m_label_gap * i,
					m_vertical_padding + m_gap + m_box_rect_height + m_gap
							* 0.5f, 2, m_label_paint);
		}
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		setMeasuredDimension(widthMeasureSpec | MeasureSpec.EXACTLY, height
				| MeasureSpec.EXACTLY);
	}
