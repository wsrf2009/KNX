...
public class MySliderSwitch extends ControlView {
	
	private Context context;
	private KNXSliderSwitch mKNXSliderSwitch;
	
	public final static long REPEAT_CMD_INTERVAL = 500;
	...
	public MySliderSwitch(Context context) {
		super(context);
	}
	
	public MySliderSwitch(Context context, AttributeSet attrs) {
		this(context, attrs, R.attr.sliderSwitchStyleAttr);
	}

	public MySliderSwitch(Context context, KNXSliderSwitch mKNXSliderSwitch) {
		this(context, null, R.attr.styleMySilder);
		this.context= context;
		setKNXControl(mKNXSliderSwitch);
		if (mKNXSliderSwitch != null) {
			this.mKNXSliderSwitch = mKNXSliderSwitch;
			initControl(mKNXSliderSwitch);
		}
	}
	
	public MySliderSwitch(Context context, AttributeSet attrs, int defStyle) {
		...
		mSliderSwitchButtonText = a.getText(R.styleable.MySliderSwitch_sliderSwitchButtonText);  //SliderSwitch名称
		if(mSliderSwitchButtonText == null) {
			mSliderSwitchButtonText = "测试";
		}
		mDrawableLeft = a.getDrawable(R.styleable.MySliderSwitch_drawableLeft);  //SliderSwitch左边背景
	...
	private void initControl(final KNXSliderSwitch mKNXSliderSwitch) {
		if(mKNXSliderSwitch != null) {
			this.setId(mKNXSliderSwitch.getId());
			setSliderSwitchButtonText(mKNXSliderSwitch.getText());
			...
			case TOUCH_MODE_DRAGGING: {
				...
				if (mOrientation == HORIZONTAL) {
					...
					if (newPos != mThumbPosition) {
						...
						if(now.getTimeInMillis() - lastUpdateTime.getTimeInMillis() > REPEAT_CMD_INTERVAL)
						{
							//发送数据
						...
						this.postDelayed(new Runnable() {
				            ...
				            public void run() {
				            	//发送数据
				            	sendCommandRequest(mKNXSliderSwitch.getETSID(), String.valueOf((int)(mThumbPosition)));
				            }
				        }, REPEAT_CMD_INTERVAL); 
						*/
					}
	...
	protected void onDraw(Canvas canvas) { 
		...
		} else if(mKNXSliderSwitch.getSliderSymbol() ==  ControlSymbol.DownUp.getControlSymbol()) {
	...
	public interface OnSliderChangeListener { 
		public abstract void onProgressChanged(MySliderSwitch slider, int progress, boolean fromUser);
