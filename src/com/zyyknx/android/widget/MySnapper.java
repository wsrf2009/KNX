...
import com.zyyknx.android.control.KNXSnapper; 

public class MySnapper extends ControlView {
	
	private Context context;
	private KNXSnapper mKNXSnapper;
	...
	public MySnapper(Context context) {
		super(context);
	}
	
	public MySnapper(Context context, AttributeSet attrs) {
		this(context, attrs, R.attr.sliderSwitchStyleAttr);
	}

	public MySnapper(Context context, KNXSnapper mKNXSnapper) {
		this(context, null, R.attr.styleMySilder);
		this.context= context;
		setKNXControl(mKNXSnapper);
		if (mKNXSnapper != null) {
			this.mKNXSnapper = mKNXSnapper;
			initControl(mKNXSnapper);
		}
	}
	
	public MySnapper(Context context, AttributeSet attrs, int defStyle) {
		...
		mOrientation = a.getInteger(R.styleable.MySnapper_direct, HORIZONTAL); //目前只支持水平
		mThumbDrawable = a.getDrawable(R.styleable.MySnapper_thumb);  //滑动条Drawable
		...
		mSliderSwitchButtonText = a.getText(R.styleable.MySnapper_sliderSwitchButtonText);  //SliderSwitch名称
		if(mSliderSwitchButtonText == null) {
			mSliderSwitchButtonText = "测试";
		}
		mDrawableLeft = a.getDrawable(R.styleable.MySnapper_drawableLeft);  //SliderSwitch左边背景
		if(mDrawableLeft == null) {
			mDrawableLeft = context.getResources().getDrawable(R.drawable.shixun_silder_swich_light_down);
		}
		mDrawableRight = a.getDrawable(R.styleable.MySnapper_drawableRight); //SliderSwitch右边背景
		if(mDrawableRight == null) {
			mDrawableRight = context.getResources().getDrawable(R.drawable.shixun_silder_swich_light_down);
		}
		mThumbTextPadding = a.getDimensionPixelSize(R.styleable.MySnapper_thumbTextPadding, 0);
		 
		mSwitchMinWidth = a.getDimensionPixelSize(R.styleable.MySnapper_switchMinWidth, 0);
		mSwitchMinHeight = a.getDimensionPixelSize(R.styleable.MySnapper_switchMinHeight, 0);
		mSwitchPadding = a.getDimensionPixelSize(R.styleable.MySnapper_switchPadding, 0);
		...
		int appearance = a.getResourceId(R.styleable.MySnapper_switchTextAppearanceAttrib, 0);
		if (appearance != 0) {
	...
	private void initControl(final KNXSnapper mKNXSnapper) {
		if(mKNXSnapper != null) {
			setSliderSwitchButtonText(mKNXSnapper.getText());
			
			MinValue = mKNXSnapper.getMinValue();
			MaxValue = mKNXSnapper.getMaxValue();
			...
			case TOUCH_MODE_DRAGGING: {
				...
				if (mOrientation == HORIZONTAL) {
					...
					if (newPos != mThumbPosition) {
						...
						//发送数据
						sendCommandRequest(mKNXSnapper.getWriteAddressIds(), String.valueOf((int)(mThumbPosition)), false, null); 
					}
					return true;
				} 
			}
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mSliderSwitchWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, ZyyKNXConstant.CONTROL_DEFAULT_WIDTH, getResources().getDisplayMetrics()); 
	...
	protected void onDraw(Canvas canvas) { 
		...
		if(mKNXSnapper.getSliderSymbol() == ControlSymbol.DardBright.getControlSymbol()) {
			if(mProgress > 0) {
		...
		} else if(mKNXSnapper.getSliderSymbol() ==  ControlSymbol.DownUp.getControlSymbol()) {
	...
	public interface OnSliderChangeListener { 
		public abstract void onProgressChanged(MySnapper slider, int progress, boolean fromUser);
