...
public class HttpSoap implements Runnable {
    ...
    private CamerDevice mCamer;
    private String mCreated, mNonce, mAuthPwd;

    public HttpSoap(CamerDevice device) {
        mCamer = device;
        createAuthString();
    }
    ...
    private void createAuthString() {
        ...
        mNonce = getNonce();
        mAuthPwd = getPasswordEncode(mNonce, mCamer.password, mCreated);
    }
    ...
    public void run() {
        try {
            // =========获取子服务=========
            initConn(mCamer.serviceURL);
            mUrlConn.connect();
            String content = String.format(GET_CAPABILITIES, mCamer.username,
            ...
            initConn(mediaUrl);
            mUrlConn.connect();
            content = String.format(GET_PROFILES, mCamer.username,
                    mAuthPwd, mNonce, mCreated);
            Log.i(TAG, "content:"+content);
            mUrlConn.getOutputStream().write(content.getBytes());
            ...
            initConn(mediaUrl);
            mUrlConn.connect();
            content = String.format(GET_PROFILE, mCamer.username, mAuthPwd,
            ...
            Log.i(TAG, "uri:"+uri);
            if (mListener != null) {
                mListener.OnHttpSoapDone(mCamer, uri, true);
            }
        } catch (Exception e) {
            if (mListener != null) {
                mListener.OnHttpSoapDone(mCamer, "", false);
            }
            e.printStackTrace();
        }
    }
                ...
                switch (eventType) {
                    ...
                    case XmlPullParser.START_TAG:
                        if (parser.getName().equals("Width")) {
                            ...
                            mCamer.width = Integer.parseInt(parser.getText());
                        } else if (parser.getName().equals("Height")) {
                            eventType = parser.next();
                            mCamer.height = Integer.parseInt(parser.getText());
                        } else if (parser.getName().equals("FrameRateLimit")) {
                            eventType = parser.next();
                            mCamer.rate = Integer.parseInt(parser.getText());
                        }
//                        Log.i(TAG, "width:"+mCamer.width+" height:"+mCamer.height+" rate:"+mCamer.rate);
//                        Log.i(TAG, "xml:"+xml);
                        break;
    ...
    private String getURIContent(String profile) {
        String content = String.format(GET_URI_BODY, mCamer.username,
...
interface OnHttpSoapListener {
    public void OnHttpSoapDone(CamerDevice camera, String uri, boolean success);
}
