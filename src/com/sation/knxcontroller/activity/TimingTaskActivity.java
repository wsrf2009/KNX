							...
							if(actionIsValid) { // action name 有效
								...
								if(actionIsValid) {
									if (1 == addressAndAction.getAddress().getType()) { // 1 Bit
						                ...
						                if ((KNXDatapointType.DPT_5 == addressAndAction.getAddress().getKnxMainNumber()) && 
						                		(KNXDatapointType.DPST_1 == addressAndAction.getAddress().getKnxSubNumber())) { // 5.001
						                    if ((0 > value) || (255 < value)) {
						                    	actionIsValid = false;
						                    }
						                } else if ((KNXDatapointType.DPT_18 == addressAndAction.getAddress().getKnxMainNumber()) &&
						                		(KNXDatapointType.DPST_1 == addressAndAction.getAddress().getKnxSubNumber())) { // 18.001
						                    if ((0 > value) || (255 < value)) {
						                    	actionIsValid = false;
						                    }
						                }
